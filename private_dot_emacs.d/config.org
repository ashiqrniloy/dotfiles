...
#+begin_src emacs-lisp :tangle "init.el"
    (defun arn/make-frame-transparent ()
      "Function to make the frame transparent"
      (interactive)
      (set-frame-parameter nil 'alpha-background 60)
      (add-to-list 'default-frame-alist '(alpha-background . 60))
      (add-to-list 'pop-up-frame-alist '(alpha-background . 100)))
...
** In buffer completion with Company mode
:PROPERTIES:
:ID:       566f7820-9af5-4a33-9714-ae5d67f763e0
:CREATED:  [2025-03-09 Sun 01:52]
:END:
For in buffer completion, Company mode provides a lot more options. For example: it can provide completion for python notebooks that work with ipython kernel. Also for Bengali writing this would be useful. However, the main reason to explore this is aesthetic. For Corfu the problem of child frame being transparent is a bad user experience. Exploring if Company mode is a good replacement for Corfu.

The company mode overlay doesn't look that good either. Mostly because it somehow changes the main buffer a little bit making it a bit wonky to use.

However, I think the main advantage of using company is that it provides a lot of third party backends such as: latex, yasnippet etc. For now I think it is best to go with company for these reasons.
#+begin_src emacs-lisp :tangle "init.el"
  (use-package company
    :ensure t
    :config
...
#+end_src
** SVG child frames
One issue with the child frames in emacs is that it inherits the properties of the parent frame. One of the problems that this creates is that if for example alpha-background is set, the child frames for completion with corfu becomes also transparent. This makes the child frame not very legible as more often than not in the area the child frame is created, there is already text there. Tes
#+begin_src emacs-lisp :tangle "init.el"
  ;; this is required for nova to work
...
#+end_src
The above code doesn't work because emacs 30.1 has a bug where it gives the error that "Can't read the whole string". Once this is fixed, the manual configuration below will be removed.
#+begin_src emacs-lisp :tangle "init.el"
;;  (add-to-list 'load-path "/home/arn/.emacs.d/nova/")
;;  (require 'nova)
;;  (require 'nova-corfu)
;;(require 'nova-eldoc)
...
;;(nova-eldoc-mode 1)
#+end_src
The code above also doesn't really work and it behaves in a strange way. Revisit this package in the future to see if this can be solved and it actually works. For now stuck with the transparency problem with corfu child frames. Anyhow it doesn't look that good.
